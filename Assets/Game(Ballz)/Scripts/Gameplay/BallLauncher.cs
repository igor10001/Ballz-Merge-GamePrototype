...
public class BallLauncher : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GameManager_ballz.Instance.m_GameState == GameManager_ballz.GameState.MainMenu || GameManager_ballz.Instance.m_GameState == GameManager_ballz.GameState.GameOver)
            return;

        if (!m_CanPlay)
            return;

        if(Time.timeScale != 0 && GameManager_ballz.Instance.m_GameState != GameManager_ballz.GameState.GameOver)
